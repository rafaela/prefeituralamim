...
export class ListaLicitacoesComponent  implements OnInit{
  ...
  buscaDados(){
    this.api.getLicitacoes(this.data).subscribe(data => {
      this.licitacoesList = data.data;
      this.total = data.total;

      this.licitacoesList.forEach((element: any) => {
  ...
  delete(id: any, enterAnimationDuration: any, exitAnimationDuration: any){
    ...
    dialogRef.afterClosed().subscribe((data: any) =>{
      if(data){
        this.api.deleteLicitacoes(this.id).subscribe((data: any) => {
          if(data.sucess){
            this.ui.sucess('', 'Licitação removida')
            this.buscaDados();
          }
          else
  ...
  inserir(){
    this.router.navigate(['/cadastro-licitacao']);
  }
