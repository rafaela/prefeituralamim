...
@Component({
  selector: 'app-lista-decretos',
  imports: [MenuComponent, NgFor, FormsModule],
  templateUrl: './lista-decretos.component.html',
  styleUrl: './lista-decretos.component.scss'
})
export class ListaDecretosComponent  implements OnInit{
  ...
  buscaDados(){
    this.api.getDecretos(this.data).subscribe(data => {
      this.decretosList = data.data;
      this.total = data.total;

      this.decretosList.forEach((element: any) => {
  ...
  delete(id: any, enterAnimationDuration: any, exitAnimationDuration: any){
    ...
    dialogRef.afterClosed().subscribe((data: any) =>{
      if(data){
        this.api.deleteDecreto(this.id).subscribe((data: any) => {
          if(data.sucess){
            this.ui.sucess('', 'Decreto removido')
            this.buscaDados();
          }
          else
  ...
  inserir(){
    this.router.navigate(['/cadastro-decretos']);
  }
